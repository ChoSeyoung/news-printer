...
중요: 크롤링한 기사가 아닌 방송용 뉴스 대본처럼 작성하세요.
원본 기사의 모든 메타정보, 특수문자, 한자를 제거하고 깨끗한 한글 대본만 생성하세요.

대본 작성 규칙:
- anchor: 첫3초에 핵심 전달, 기자연결멘트 제외
- reporter: 견해/분석 포함, 반드시 완결된 서술어로 끝나야 함
  * 마지막 문장 반드시 서술어로 종결: "~습니다", "~했습니다", "~알려졌습니다", "~보입니다", "~전망입니다", "~예정입니다", "~것으로 분석됩니다"
  * 절대 금지 1: 불완전한 문장 ("여야는,", "정부는,", "대통령이," 등 주어만 있는 문장)
  * 절대 금지 2: 조사로 끝나는 문장 ("~는", "~가", "~를", "~이", "~와", "~에서" 등)
  * 절대 금지 3: 송출멘트 ("~였습니다. 지금까지...", "~입니다. OOO기자", 기자 이름)
  * reporter 작성 완료 전 반드시 확인: 마지막 단어가 서술어(동사/형용사)인가?
- 방송 뉴스 앵커가 읽는 자연스러운 한글 문체
- 한 문장은 최대 35글자 이내로 작성 (자막 가독성)
- 긴 문장은 여러 짧은 문장으로 분리
- 숫자는 한글로 풀어쓰기 (예: 3개 → 세 개, 2025년 → 이천이십오년)
- 절대 제목을 읽지 말 것, 바로 본문 핵심 내용으로 시작
- 제목 언급 금지, 제목 내용 반복 금지

한자 및 이니셜 변환 (반드시 한글로):
- 尹 → 윤석열, 李 → 이재명, 盧 → 노무현
- YS → 김영삼, DJ → 김대중, MB → 이명박, MH → 노무현, JP → 김종필
- 與 → 여당, 野 → 야당, 靑 → 청와대, 檢 → 검찰
- 前 → 전, 後 → 후, 中 → 중, 韓 → 한, 美 → 미, 日 → 일
- 北 → 북, 南 → 남, 東 → 동, 西 → 서, 大 → 대, 小 → 소

절대 포함하지 말 것:
- 기자 이름이나 기자라는 단어 (예: 홍길동 기자, xxx 기자)
- 기사 출처 정보 (예: 서울=뉴스1, 부산=연합뉴스)
- 지역이나 언론사 표기 (예: (서울=뉴스1), (부산=연합))
- 괄호 안의 부가 정보나 영문 표기
- 모든 특수문자 (온점, 쉼표, 느낌표, 물음표만 사용)
- 따옴표, 괄호, 하이픈, 슬래시 등 모든 기호
- 한자, 영문 이니셜 (반드시 위 변환 규칙에 따라 한글로)
...
- 짧고 명확한 문장으로 구성
- ~입니다 종결어미 사용
- 내용을 자연스럽게 마무리 (단, 형식적 인사말이나 송출멘트 제외)
- 방송 뉴스 앵커가 읽는 자연스러운 한글 문체
- 한 문장은 최대 35글자 이내로 작성 (자막 가독성)
- 긴 문장은 여러 짧은 문장으로 분리
- 숫자는 한글로 풀어쓰기 (예: 3개 → 세 개, 2025년 → 이천이십오년)
- 절대 제목을 읽지 말 것, 바로 본문 핵심 내용으로 시작
- 제목 언급 금지, 제목 내용 반복 금지
- 마지막 문장은 완결된 형태로 끝맺음 (예: ~습니다, ~것으로 보입니다, ~전망입니다)
